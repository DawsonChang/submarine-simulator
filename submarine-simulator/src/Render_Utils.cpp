...
void Core::RenderContextFile::initFromFile(const float* boxPositions, int size) {
    vertexArray = 0;
    vertexBuffer = 0;
    ...
    glBufferSubData(GL_ARRAY_BUFFER, 0, size, &boxPositions[0]);
    glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 0, BUFFER_OFFSET(0));

    glBindVertexArray(0);
}

void Core::RenderContext::initFromOBJWithBump(obj::Model& model)
{
    vertexArray = 0;
    vertexBuffer = 0;
    vertexIndexBuffer = 0;
